...
import functools
import re
from typing import Iterable, NewType, Optional, Self, Union, assert_type, cast
...
_campaign_id_pattern = re.compile(r"^c([a-fA-F0-9]{4})$")
...
class CampaignState(Storable):
    ...
    def add_campaign(self, camp: Campaign):
        ...
        if not self.validate_campaign_id(camp_id):
            raise ValueError(f"invalid campaign id: {camp_id}")
    ...
    def validate_campaign_id(self, cid: CampaignId) -> Optional[re.Match]:
        return _campaign_id_pattern.fullmatch(cid)

    def generate_new_id(self) -> CampaignId:
        if self.campaigns:
            ints = list(map(lambda x: int(x[1:], base=16), self.campaigns))
            new_num = max(ints) + 1
        else:
            new_num = 0
        return CampaignId(f"c{new_num:04x}")
